...
public class ChangeSecondPic extends HttpServlet {
	
	private static final long serialVersionUID = 1L;
       
    public ChangeSecondPic() {
        super();
        }
		...
		try {
			...
			while(rs.next()){
				...
				if(db_usr.equals(usr)&&db_pwd.equals(pwd)){
					...
					RequestDispatcher view = request.getRequestDispatcher("changesecondpicform.jsp");
	    			view.forward(request,response);
				}
			}
			out.print("You need to have profile to change the second pic");
